...
Additional targets: `make fmt` (`gofmt -w .`) and `make vet` (`go vet ./...`).
...
go run ./cmd/bench     -csv btc_orderbook_updates.csv -iters 100 -out BENCHMARK.md
```
...
| `-symbol` | string | `BTC/USDT` | Expected symbol. |
| `-iters` | int | `50` | Measured full-fixture replay iterations. |
| `-warmup` | int | `5` | Warmup iterations discarded before measurement. |
| `-ring` | int (power of two) | `4096` | Book-event ring capacity used during the run. |
| `-log-ring` | int (power of two) | `4096` | Log ring capacity used during the run. |
...
See **[DESIGN.md](DESIGN.md)** for the full design, state machine, and rationale.
...
- **Timestamp-step continuity is fixture-specific.** This verified synthetic
  ...
  `UpdateIDPolicy` seam in DESIGN.md.
...
- **[DESIGN.md](DESIGN.md)** — full design: architecture, book structures, sync
